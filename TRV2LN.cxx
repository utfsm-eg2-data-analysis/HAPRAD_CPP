...
TRV2LN::TRV2LN(const TRadCor* rc, double phi_k) : fH(rc), fPhiK(phi_k) {
  fRC = rc;
...
ROOT::Math::IBaseFunctionOneDim* TRV2LN::Clone() const { return 0; }
...
double TRV2LN::DoEval(double tauln) const {
  ...
  mu = (fHadKin->Eh() - fHadKin->Pl() * costk - fHadKin->Pt() * sintk * TMath::Cos(fPhiK - fKin->PhiH())) / M;
  factor = 1. + tau - mu;
