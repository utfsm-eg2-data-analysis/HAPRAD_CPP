...
class TThetaMatrix {
 public:
  ...
  void Evaluate(Double_t tau, Double_t mu, Int_t ita, Double_t phi_k);

  Double_t& operator()(unsigned row, unsigned col) { return fData[fCols * row + col]; };
  Double_t operator()(unsigned row, unsigned col) const { return fData[fCols * row + col]; };
