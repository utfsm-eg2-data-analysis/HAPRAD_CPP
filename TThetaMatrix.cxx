...
TThetaMatrix::TThetaMatrix(const TRadCor* rc) : fRows(4), fCols(3) {
...
TThetaMatrix::TThetaMatrix(Int_t rows, Int_t cols, const TRadCor* rc) : fRows(rows), fCols(cols) {
  fConfig = rc->GetConfig();
  fKin = rc->GetKinematicalVariables();
  fInv = rc->GetLorentzInvariants();
  fHadKin = rc->GetHadronKinematics();
...
void TThetaMatrix::Evaluate(Double_t tau, Double_t mu, Int_t ita, Double_t phi_k) {
  ...
  if (fConfig->IntegratePhiRad() == 0 && ita == 1) {
    ...
    z1 = (Q2 * Sp + tau * (S * Sx + 2 * M2 * Q2) - 2 * M * Cos(phi_k) * sqrtmb) / Lq;
    z2 = (Q2 * Sp + tau * (X * Sx - 2 * M2 * Q2) - 2 * M * Cos(phi_k) * sqrtmb) / Lq;
    bb = 1;
    bi12 = bb / (z1 * z2);
    bi1pi2 = bb / z2 + bb / z1;
  ...
  (*this)(1, 1) = 0.5 * (bi1pi2 * Sx * Sp - bi12 * tau * SQ(Sp) + 2. * bir * Sp + 2. * hi2 * (Sx - 2. * M2 * tau));
  (*this)(1, 2) = 0.5 * (bi12 * tau * (2. * M2 * tau - Sx) - bi1pi2 * Sp + 4. * M2 * bb);

  (*this)(2, 0) = 2. * hi2 * (V1 * V2 - mh2 * Q2);
  (*this)(2, 1) = -2. * ((mh2 * tau - mu * vvm) * hi2 - bir * mu * vvp - bi1pi2 * vvm * vvp + bi12 * tau * SQ(vvp));
  (*this)(2, 2) = bi12 * tau * (mh2 * tau - mu * vvm) - bi1pi2 * mu * vvp + 2. * mh2 * bb;

  (*this)(3, 0) = -2. * (V1 * Sx - 2. * S * vvp + Q2 * Sx * zh) * hi2;
  (*this)(3, 1) = -2. * bi12 * tau * vvp * Sp + bi1pi2 * (Sp * vvm + Sx * vvp) + bir * (mu * Sp + 2. * vvp) +
                  hi2 * (Sx * (mu - 2. * tau * zh) + 2. * vvm);
  (*this)(3, 2) = bi12 * tau * (Sx * (tau * zh - mu / 2.) - vvm) - bi1pi2 * (Sp * mu / 2. + vvp) + 2. * Sx * zh * bb;
}
