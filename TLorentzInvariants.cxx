...
TLorentzInvariants::TLorentzInvariants(THapradConfig* config, TKinematicalVariables* kin)
    : fConfig(config), fKin(kin), fHadKin(0), fS(0), fX(0), fSx(0), fSp(0), fQ2(0) {
  // Do nothing
}
...
void TLorentzInvariants::SetS() { fS = 2 * kMassProton * fKin->E(); }

void TLorentzInvariants::SetX() { fX = fS * (1 - fKin->Y()); }

void TLorentzInvariants::SetSx() { fSx = fS - fX; }

void TLorentzInvariants::SetSp() { fSp = fS + fX; }
...
void TLorentzInvariants::SetQ2() {
  ...
  if (fKin->Y() > y_max || fKin->Y() < y_min || fKin->X() > 1 || fKin->X() < 0) {
    std::cout << "    y: " << fKin->Y() << "    x: " << fKin->X() << std::endl;
    throw TKinematicException();
  }
}

void TLorentzInvariants::SetW2() { fW2 = fS - fX - fQ2 + SQ(kMassProton); }
