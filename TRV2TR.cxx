...
ROOT::Math::IBaseFunctionMultiDim* TRV2TR::Clone() const { return 0; }

unsigned int TRV2TR::NDim() const { return 2; }
...
double TRV2TR::DoEval(const double* x) const {
  ...
  double mu =
      (fHadKin->Eh() - fHadKin->Pl() * (fInv->Sx() - tau * 2 * M2) / fInv->SqrtLq() -
       2 * M2 * fHadKin->Pt() * TMath::Sqrt((tau - tau_min) * (tau_max - tau)) * TMath::Cos(fKin->PhiH() - phi_k) / fInv->SqrtLq()) /
      M;
