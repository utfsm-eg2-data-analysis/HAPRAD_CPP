...
THadronKinematics::THadronKinematics(THapradConfig* config, TKinematicalVariables* kin, TLorentzInvariants* inv)
    : fConfig(config), fKin(kin), fInv(inv), fEh(0), fPl(0), fPt(0), fNu(0), fPx2(0), fPh(0) {
  // Do nothing
}
...
void THadronKinematics::SetNu(void) { fNu = fInv->Sx() / (2 * kMassProton); }

void THadronKinematics::SetEh(void) {
  fEh = fNu * fKin->Z();

  if (fEh < kMassDetectedHadron) {
    std::cout << "    E_h: " << fEh << std::endl;
    throw TKinematicException();
  }
}

void THadronKinematics::SetSqNuQ(void) { fSqNuQ = TMath::Sqrt(SQ(fNu) + fInv->Q2()); }
...
void THadronKinematics::SetMomentum(void) {
  ...
  if (fKin->T() >= 0) {
    ...
    if (fPh < fPt) {
      std::cout << "    p_h: " << fPh << "    p_t: " << fPt << std::endl;
      throw TKinematicException();
    }
    ...
    if (fPh < TMath::Abs(fPl)) {
      ...
      if (TMath::Abs(mineps) > epspl) {
        std::cout << "    p_h: " << fPh << "    p_l: " << fPl << std::endl;
        throw TKinematicException();
      } else {
        std::cout << "    Zero p_t! " << fPl << "\t" << mineps << "\t" << epspl << std::endl;
  ...
  if ((fKin->T() - t_min) > kEpsMachine || fKin->T() < t_max) {
    std::cout << "    t:     " << fKin->T() << "    t_min: " << t_min << "    t_max: " << t_max << std::endl;
    throw TKinematicException();
  }
}
...
void THadronKinematics::SetPx2(void) {
  ...
  if (fPx2 < kMassC2 || fPx2 > px2_max) {
    std::cout << "    px2:    " << fPx2 << "    mc2:    " << kMassC2 << "    px2max: " << px2_max << std::endl;
    throw TKinematicException();
  }
}
