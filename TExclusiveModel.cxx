...
void ExclusiveModel(Double_t q2m, Double_t wm, Double_t csthcm, Double_t& st, Double_t& sl, Double_t& stt, Double_t& stl, Double_t& stlp) {
  Double_t q2_pn[nq] = {0.0, 0.3, 0.6, 0.9, 1.2, 1.5, 1.8, 2.1, 2.4, 2.7, 3.0, 3.3, 3.6, 3.9, 4.2, 4.5, 4.8, 5.0};

  Double_t w_pn[nw] = {1.08, 1.10, 1.12, 1.14, 1.16, 1.18, 1.20, 1.22, 1.24, 1.26, 1.28, 1.30, 1.32, 1.34, 1.36, 1.38,
                       1.40, 1.42, 1.44, 1.46, 1.48, 1.50, 1.52, 1.54, 1.56, 1.58, 1.60, 1.62, 1.64, 1.66, 1.68, 1.70,
                       1.72, 1.74, 1.76, 1.78, 1.80, 1.82, 1.84, 1.86, 1.88, 1.90, 1.92, 1.94, 1.96, 1.98, 2.00};

  Double_t th_cm_pn[nt] = {0.,   3.,   6.,   9.,   12.,  15.,  18.,  21.,  24.,  27.,  30.,  33.,  36.,  39.,  42.,  45.,
  ...
  if (nc == 0) {
    ...
    for (Int_t j = 0; j < nw; j++) rarg[j + nq] = w_pn[j];

    for (Int_t k = 0; k < nt; k++) rarg[k + nq + nw] = th_cm_pn[k];
